...
def sendEmail(parameters):
    response = {}
    sent = MainClass.sendEmail(parameters["emailTo"],parameters["message"],parameters["subject"])
    if sent:
...
def saveReport(details):
    response = {}
    conn = pymssql.connect(os.environ['server'], os.environ['user'], os.environ['password'], os.environ['db'])
...
def updateReport(parameters):
    ...
    seconds = int((datetime.datetime.now() - call["startCall"]).total_seconds())
    cursor.execute("UPDATE calls set [endCall]=%s, [offset]=%s WHERE [id] =%s",(str(datetime.datetime.now())[:-7]), seconds)
    conn.commit()
    response["result"] = True
    return response
